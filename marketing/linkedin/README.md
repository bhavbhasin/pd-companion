...
- [ ] **Magical moments — tremor chart · observations panel · food chips · insights · (predictions)** — a "wow surfaces" showcase. tremor chart / observations panel / food chips / insights are all SHIPPED (food chips live in build 5) = real screenshots. ⚠ Predictions has NO shipping UI yet (the Tier-1 "what you can expect today" card is designed, not built) — drop it from the showcase OR label it "coming," never imply it exists/is screenshot-able. Overlaps posted #3 (insights) — give it fresh framing. — `ready` (minus predictions)
