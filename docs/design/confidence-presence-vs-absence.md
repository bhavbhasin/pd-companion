...
- **Worked example (the reference user, Jul 2026 — computed from the Jul 6 CSV export, `walking_speed_m_s`, foreign-device samples excluded):** 71 monthly medians, 5.83 y span, baseline 1.001 m/s. Slope **+0.70 cm/s/yr** (SE 0.39, t=1.8 — *not* significant, which is exactly why the presence gate capped this card at Emerging). Total change **+4.10 cm/s (+4.1%)**, lower bound −0.4 cm/s. Own detectable margin **0.045 m/s < MCID 0.060** ⇒ this user can resolve a clinically meaningful change, so the MCID binds. Non-inferiority: t = (0.041+0.06)/0.0225 ≈ **4.5**, one-sided p ≈ **1.4e−5** → **Strong**. ✅ reproduces the shipped card's +4%.
...
5. ✅ the reference user's own-data gait margin computed — **0.037 m/s, so the MCID binds**. See the worked example above.
...
Related: `feedback_no_arbitrary_thresholds`, `observation_accuracy`, `docs/intelligence-architecture.md` (engine-judges layer). **Next project (the reference user, Jul 13):** build out all dormant renderers so any valid correlation lights up as soon as it's earned — the "delightful secret sauce." Scope after this confidence work lands.
